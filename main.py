                ...
                try:
                    ...
                    async with aiohttp.ClientSession(timeout=timeout, connector=connector) as session:
                        async with session.post(
                            ...
                            # 尝试多种格式的图片URL（兼容不同平台返回的Markdown）
                            image_url_match = re.search(r'\[点击下载\]\((https?://[^\s)]+)\)', content)
                            if not image_url_match:
                                # 兼容任意 alt 文本的 Markdown 图片，如 ![image](...)、![图片](...)
...
class MainWindow(QMainWindow):
    ...
    def create_generation_card(self, parent_layout):
        ...
        # 停止生成按钮
        self.stop_generation_button = QPushButton("⏹️ 停止生成")
        self.stop_generation_button.setMinimumHeight(50)
    ...
    def stop_generation(self):
        ...
        # 更新状态显示
        self.overall_progress_label.setText("⏹️ 生成已停止")
        self.overall_progress_bar.setVisible(False)
        
        # 记录停止日志
        logging.info("=== 用户手动停止生成 ===")
        
        QMessageBox.information(self, "停止完成", "生成任务已停止")
